...
export function ShieldIcon({ size = 14 }) {
  const id = ++_counter
  const gradId = `dhi-g-${id}`
  const maskId = `dhi-m-${id}`

  return (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none" style={{ flexShrink: 0 }}>
      <defs>
        <linearGradient id={gradId} x1="-1.52" y1="-1.26" x2="24.21" y2="3.61" gradientUnits="userSpaceOnUse">
        ...
        <mask id={maskId} style={{ maskType: 'alpha' }} maskUnits="userSpaceOnUse" x="2" y="0" width="17" height="20">
          <path
      ...
      <g mask={`url(#${maskId})`}>
        <rect width="20" height="20" fill={`url(#${gradId})`} />
      </g>
    </svg>
  )
}

export function DhiBadge({ size = 14, label = 'Docker Hardened Image', fontSize = '0.75rem' }) {
  return (
