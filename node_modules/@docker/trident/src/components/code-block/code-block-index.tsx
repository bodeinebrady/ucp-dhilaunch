...
/**
 * This would be a standalone import from `@radix-ui/react-use-controllable-state`
 * That package is an internal package. Since we're using `radix-ui` instead of
 * explicit individual dependencies, we need to import it this way.
 */
...
/**
 * Tree-shaking note: every icon imported above is referenced as a value in
 * this map, so bundlers cannot drop any of them. Even though the runtime
 * lookup below picks exactly one icon per filename, the static reference
 * here pins the entire set into the consumer's bundle. React Icons' "only
 * the icons you import" guarantee is import-level only — it does not help
 * once they are all held in a module-scoped map like this.
 *
 * If we want true per-consumer tree-shaking, options are:
 *   1. Drop this map and require callers to pass `icon` explicitly via
 *      CodeBlockFilenameProps['icon']. Cleanest, but loses auto-detect.
 *   2. Lazy-load icons with dynamic import() per pattern. Keeps auto-detect
 *      but splits each icon into its own chunk (first-render cost).
 *   3. Build-time codegen the map from actual usage in the consuming app
 *      (e.g. a Vite plugin). Most machinery, least disruption to the API.
 */
...
/**
 * Sort patterns by specificity (literal character count, descending) so that
 * `*.test.js` wins over `*.js`, `mintlify.json` wins over `*.json`, etc.
 * Iteration order of the map alone is not safe to rely on for correctness.
 */
const filenameIconEntries = Object.entries(filenameIconMap).sort(
...
export const CodeBlockFilename = ({
  ...
  const defaultIcon = filenameIconEntries.find(([pattern]) => {
...
type CodeBlockFallbackProps = Omit<
  HTMLAttributes<HTMLDivElement>,
  'children'
> & {
  children: string;
